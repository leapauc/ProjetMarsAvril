    ...
    <div class="container">
      ...
      <div class="admin-header anim-up">
        <div class="admin-title-wrap">
          ...
          <div>
            ...
            <p class="admin-sub">Gestion globale de la plateforme EventFlow</p>
          </div>
        </div>
        <!-- Stats rapides -->
      ...
      <div v-else-if="tab === 'events'">
        ...
        <div class="toolbar anim-up d2">
          <input v-model="eventSearch" class="search-input" placeholder="🔍 Rechercher un événement..." />
        ...
        <div class="items-list anim-up d2">
          <div v-for="(ev, i) in filteredEvents" :key="ev.id_event"
            class="item-card card" :class="`d${Math.min(i + 2, 6)}`">
            <div class="item-info">
              <div class="item-top">
              ...
              <p class="item-meta">
                ...
                <span class="orga-chip">par {{ ev.orga_firstname }} {{ ev.orga_lastname }}</span>
              </p>
            </div>
            <div class="item-actions">
              <RouterLink :to="`/events/${ev.id_event}`" class="btn btn-ghost btn-sm">Voir</RouterLink>
              <button class="btn btn-danger btn-sm" @click="openDeleteEventModal(ev)">Supprimer</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="tab === 'users'">
        <div class="toolbar anim-up d2">
          <input v-model="userSearch" class="search-input" placeholder="🔍 Nom, email..." />
        ...
        <div class="items-list anim-up d2">
          <div v-for="(u, i) in filteredUsers" :key="u.id_user"
            class="item-card card" :class="`d${Math.min(i + 2, 6)}`">
            <div class="item-info">
              <div class="item-top">
                <span class="badge" :class="roleClass(u.role)">{{ u.role }}</span>
                <span v-if="u.is_anonymized" class="badge badge-err">Anonymisé</span>
                <span class="item-date">Inscrit le {{ formatDate(u.created_at) }}</span>
              </div>
              <h3 class="item-title">
                {{ u.is_anonymized ? 'Utilisateur anonymisé' : `${u.firstname} ${u.lastname}` }}
              </h3>
              <p class="item-meta">{{ u.is_anonymized ? '—' : u.email }}</p>
            </div>
            <div class="item-actions" v-if="!u.is_anonymized">
              <!-- Changement de rôle -->
              <select class="role-select" :value="u.role" @change="openChangeRoleModal(u, $event.target.value)">
                <option value="USER">USER</option>
                <option value="ORGANIZER">ORGANIZER</option>
                <option value="ADMIN">ADMIN</option>
              </select>
              <button class="btn btn-danger btn-sm" @click="openAnonymizeModal(u)">Anonymiser</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="tab === 'activity'">
        <div class="toolbar anim-up d2">
          <input v-model="activitySearch" class="search-input" placeholder="🔍 Utilisateur, action, événement..." />
        ...
        <div class="activity-table card anim-up d2">
          <table>
            ...
            <tbody>
              <tr v-for="log in filteredActivity" :key="`${log.source}-${log.log_date}-${log.user_id}`">
                <td>{{ log.user_name || '—' }}</td>
                <td><span class="badge" :class="actionClass(log.log_type)">{{ formatAction(log.log_type) }}</span></td>
                <td class="td-event">{{ log.related_event_title || '—' }}</td>
                <td class="td-date">{{ formatDateTime(log.log_date) }}</td>
                <td><span class="source-chip">{{ log.source }}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
...
const filteredEvents = computed(() => {
  return events.value.filter(ev => {
    const matchSearch = !eventSearch.value ||
      ...
      (ev.location || '').toLowerCase().includes(eventSearch.value.toLowerCase())
    const matchFilter =
...
function roleClass(r) {
  return { USER: 'badge-muted', ORGANIZER: 'badge-primary', ADMIN: 'badge-admin' }[r] || 'badge-muted'
}
function actionClass(a) {
  if (!a) return 'badge-muted'
  if (a.includes('deleted') || a.includes('cancelled') || a.includes('refused')) return 'badge-err'
  if (a.includes('validated') || a.includes('confirmed') || a.includes('exported')) return 'badge-ok'
...
function formatAction(a) { return ACTION_LABELS[a] || a }
...
async function doAnonymize(u) {
  try {
    ...
    if (idx !== -1) users.value[idx] = { ...users.value[idx], is_anonymized: true, firstname: 'Utilisateur supprimé', lastname: 'Utilisateur supprimé', email: '—' }
...
.admin-sub { font-size: 14px; color: var(--c-text-2); }
...
.stat-label { font-size: 11px; color: var(--c-text-2); margin-top: 2px; text-align: center; }
...
.orga-chip { color: var(--c-primary); font-size: 12px; }
...
/* ── Activity table ──────────────────────────────── */
.activity-table { overflow-x: auto; padding: 0; }
table { width: 100%; border-collapse: collapse; font-size: 13px; }
thead tr { border-bottom: 1px solid var(--c-border); }
th {
  padding: 12px 16px;
  text-align: left;
  font-size: 11px;
...
.td-date { color: var(--c-text-2); white-space: nowrap; }
.td-event { max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.source-chip {
...
/* Badge admin */
.badge-admin { background: rgba(220,38,38,.15); color: #f87171; }
...
.modal-box { max-width: 440px; width: 100%; padding: 28px; }
.modal-title { font-size: 18px; font-weight: 700; margin-bottom: 12px; }
.modal-msg { font-size: 14px; color: var(--c-text-2); line-height: 1.6; margin-bottom: 24px; }
...
/* ── Empty state ─────────────────────────────────── */
.empty-state { text-align: center; padding: 60px 20px; color: var(--c-text-2); }
...
.tab-count {
  ...
  background: var(--c-primary-lite); color: var(--c-primary);
...
@media (max-width: 640px) {
  ...
  .item-actions .btn, .item-actions .role-select { flex: 1; }
