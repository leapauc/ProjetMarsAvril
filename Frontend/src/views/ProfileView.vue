    ...
    <div class="container" style="max-width:840px">
      ...
      <template v-else-if="profileData">
        ...
        <div class="profile-hero card anim-up">
          ...
          <div class="ph-info">
            <h1 class="ph-name">{{ profileData.firstname }} {{ profileData.lastname }}</h1>
            <p class="ph-email">{{ profileData.email }}</p>
            <div class="ph-badges">
              <span class="badge badge-primary">{{ profileData.role }}</span>
              <span class="badge badge-muted" v-if="profileData.is_anonymized">Anonymisé</span>
            </div>
          </div>
          <div class="ph-meta">
            <div class="ph-meta-item">
              <span class="ph-meta-val">{{ (profileData.events || []).length }}</span>
              <span class="ph-meta-label">{{ auth.isOrganizer ? 'Événements' : 'Inscriptions' }}</span>
            </div>
            <div class="ph-meta-item">
        ...
        <div class="profile-tabs anim-up d1">
          <button class="tab-btn" :class="{ active: tab === 'info' }"  @click="tab = 'info'">👤 Informations</button>
          <button class="tab-btn" :class="{ active: tab === 'rgpd' }"  @click="tab = 'rgpd'">🔒 Mes données RGPD</button>
        </div>
            ...
            <div v-if="!editMode" class="info-view">
              <div class="info-grid">
                ...
                <div class="info-field">
                  ...
                  <span class="info-val">{{ profileData.phone || 'Non renseigné' }}</span>
                </div>
                <div class="info-field">
                ...
                <div class="info-field">
                  ...
                  <span class="info-val">{{ formatDate(profileData.created_at) }}</span>
                </div>
              </div>
              <button class="btn btn-secondary" style="margin-top:8px" @click="startEdit">✏️ Modifier mes informations</button>
            </div>
        ...
        <div v-if="tab === 'rgpd'" class="tab-panel anim-up d2">
          ...
          <div class="rgpd-card card">
            ...
            <p class="rgpd-desc">Téléchargez une copie complète de vos données personnelles conformément au RGPD (droit à la portabilité).</p>
            <div class="rgpd-actions">
              <a :href="`/api/me/${auth.user.id_user}/export`" target="_blank" class="btn btn-secondary">
                📄 Exporter en JSON
              </a>
              <a :href="`/api/me/${auth.user.id_user}/export/pdf`" target="_blank" class="btn btn-secondary">
                📋 Exporter en PDF
              </a>
            </div>
          </div>
          ...
          <div class="rgpd-card card">
            ...
            <div class="consent-info">
              <div class="ci-row"><span>Version acceptée</span><strong>{{ profileData.consent_version }}</strong></div>
              <div class="ci-row"><span>Date d'acceptation</span><strong>{{ formatDate(profileData.consent_date) }}</strong></div>
            </div>
          </div>
          ...
          <div class="rgpd-card card danger-zone">
            ...
            <p class="rgpd-desc">
              L'anonymisation remplace vos données personnelles par des valeurs neutres.
              Votre compte reste actif mais vos informations seront effacées.
              <strong>Cette action est irréversible.</strong>
            </p>
            <button class="btn btn-danger" @click="showAnonymizeModal = true" :disabled="profileData.is_anonymized">
              {{ profileData.is_anonymized ? 'Compte déjà anonymisé' : '🗑️ Anonymiser mon compte' }}
            </button>
          </div>
      ...
      <Transition name="fade">
        <div v-if="showAnonymizeModal" class="modal-overlay" @click.self="showAnonymizeModal = false">
          <div class="modal-box">
            <h2 class="modal-title">⚠️ Confirmer l'anonymisation</h2>
            <p style="color:var(--c-text-2);font-size:14px;line-height:1.6">
              Votre prénom et nom seront remplacés par "Utilisateur supprimé", votre email hashé,
              votre téléphone effacé. Cette action est <strong>définitive et irréversible</strong>.
            </p>
            <div v-if="anonError" class="alert alert-error" style="margin-top:12px">{{ anonError }}</div>
            <div class="modal-actions">
              <button class="btn btn-secondary" @click="showAnonymizeModal = false">Annuler</button>
              <button class="btn btn-danger" @click="anonymize" :disabled="anonLoading">
...
const editForm = reactive({ firstname: '', lastname: '', email: '', phone: '' })

const initials = computed(() => {
  const d = profileData.value
  return d ? (d.firstname?.[0] || '') + (d.lastname?.[0] || '') : '?'
})
const consentVersion = computed(() => profileData.value?.consent_version || '—')
...
async function saveEdit() {
  ...
  try {
    ...
    setTimeout(() => { editMode.value = false }, 1200)
  } catch (e) {
...
async function anonymize() {
  ...
  try {
    ...
    anonError.value = e.response?.data?.message || 'Erreur lors de l\'anonymisation'
  } finally {
...
.ph-email { font-size: 14px; color: var(--c-text-2); margin-bottom: 8px; }
.ph-badges { display: flex; gap: 8px; }
.ph-meta { display: flex; gap: 32px; margin-left: auto; }
.ph-meta-item { text-align: center; }
.ph-meta-val   { display: block; font-size: 24px; font-weight: 800; background: var(--grad-brand); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.ph-meta-label { font-size: 11px; color: var(--c-text-2); text-transform: uppercase; letter-spacing: .06em; }
...
.info-label { font-size: 11px; font-weight: 600; color: var(--c-text-3); text-transform: uppercase; letter-spacing: .08em; }
.info-val   { font-size: 15px; color: var(--c-text); }
...
.rgpd-card { padding: 24px; }
.rgpd-section-title { font-size: 16px; font-weight: 700; margin-bottom: 10px; }
.rgpd-section-title.danger { color: var(--c-err); }
.rgpd-desc  { font-size: 14px; color: var(--c-text-2); line-height: 1.6; margin-bottom: 16px; }
...
.consent-info { display: flex; flex-direction: column; gap: 10px; }
.ci-row {
...
.ci-row strong { color: var(--c-text); }
