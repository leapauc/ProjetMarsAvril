...
describe("Events API Endpoints", () => {
  ...
  afterEach(async () => {
    if (testEventId) {
      await pool.query(
        "DELETE FROM user_action_log WHERE related_event = $1",
        [testEventId],
      );

      await pool.query(
        "DELETE FROM registrations WHERE id_event = $1",
        [testEventId],
      );
  ...
  it(
    "POST /api/registrations/:id/register - refuse une inscription sur un événement passé",
    async () => {
      let pastEventId;
      let userId;

      try {
        // Création d'un événement déjà passé
        ...
        pastEventId = pastEvent.rows[0].id_event;
        ...
        // Création du token JWT de l'utilisateur
        const userToken = jwt.sign(
          {
            id: userId,
            email: `past.user.${Date.now()}@mail.com`,
            role: "USER",
          },
          process.env.JWT_SECRET,
        );
        ...
        const res = await request(app)
          .post(`/api/registrations/${pastEventId}/register`)
        ...
        // Nettoyage même si le test échoue
        if (userId && pastEventId) {
          await pool.query(
            "DELETE FROM registrations WHERE id_user = $1 AND id_event = $2",
            [userId, pastEventId],
          );
        }
